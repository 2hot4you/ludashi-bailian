...
type TaskRequest struct {
	...
	TaskType     string         `json:"task_type" gorm:"type:varchar(100);not null"`       // i2v-first-frame, i2v-keyframes
...
type VideoGenerationRequest struct {
	TaskType     string `json:"task_type" binding:"required"` // i2v-first-frame, i2v-keyframes
	Model        string `json:"model" binding:"required"`     // 模型名称
	Prompt       string `json:"prompt" binding:"required"`    // 文本提示词（最大800字符）
	ImageURL     string `json:"image_url" binding:"required"` // 图片URL（首帧）
